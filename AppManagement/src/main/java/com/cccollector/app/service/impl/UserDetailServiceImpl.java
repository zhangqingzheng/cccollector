...
public class UserDetailServiceImpl extends GenericServiceHibernateImpl<Integer, UserDetail> implements UserDetailService, UserDetailsService {
	...
	@Override
	public Response modifyUserProfile(int userId, String email, String cellphone) {
		try {
	...
	public Response getUserModulePermissions(int userId) {
		...
		for (Permission userPermission : userPermissions) {
			...
			modules.get(moduleIdentifier).getPermissionMap().put(userPermission.getPermissionId(), userPermission);
		}
	...
	public Response loadDepartments(String code) {
		...
		orderQueryFieldList.add(new QueryField("code", true));
		List<Department> allDepartments = departmentDao.loadAll(orderQueryFieldList);
		Iterator<Department> iter = allDepartments.iterator();
        while (iter.hasNext()) {
            Department department = iter.next();
            if (department.getCode().split(":").length == 1 && !department.getCode().startsWith(code.split(":")[0])) {
            	iter.remove();
            } else if (department.getCode().split(":").length > 1 && !department.getCode().startsWith(code)) {
            	iter.remove();
            }
        }
		return Response.ok(allDepartments).build();
	}
	@Override
