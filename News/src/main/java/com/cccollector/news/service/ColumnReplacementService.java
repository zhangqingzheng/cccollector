/**
 * 
 */
package com.cccollector.news.service;

import java.util.List;

import com.cccollector.news.model.ColumnReplacement;
import com.cccollector.universal.service.GenericService;

/**
 * 栏目替代服务接口
 *
 * @author 谢朋
 * Copyright © 2015-2018年 北京华星成汇文化发展有限公司. All rights reserved.
 */
public interface ColumnReplacementService extends GenericService<Integer, ColumnReplacement> {
	
	/**
	 * 添加栏目替代
	 * 
	 * @param columnReplacement 待添加的栏目替代
	 */
	public void addColumnReplacement(ColumnReplacement columnReplacement);
	
	/**
	 * 移动栏目替代
	 * 
	 * @param columnReplacements 移动后的栏目替代
	 */
	public void moveColumnReplacements(List<ColumnReplacement> columnReplacements);
	
}
